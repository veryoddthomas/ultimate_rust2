...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!("A Frog hopped! Remaining energy: {}", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!("A Frog will go to sleep since he ran out of energy");
            self.sleep();
        }
    }
    pub fn sleep(&mut self) {
        if self.sleeping {
            ...
            error!("A Frog is already asleep");
        } else {
            self.sleeping = true;
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!("A default Frog was generated: {:?}", default_frog);
